...
/// <summary>
/// ќтвечает за автоматическую фазу атаки:
/// все атакующие карты с loyalty > 0 автоматически нанос€т урон по цел€м.
/// ”чтены пассивные способности Ђприоритетна€ цельї и триггеры OnAttack/OnDamaged.
/// </summary>
public class AttackManager : MonoBehaviour
{
    ...
    /// <summary>
    /// –азрешает фазу атаки: все свои карты с loyalty > 0 автоматически бьют по врагу.
    /// </summary>
    /// <param name="isPlayerTurn">true Ч ход игрока, false Ч ход »»</param>
    public void ResolveAttackPhase(bool isPlayerTurn)
    {
        ...
        // нет атакующих или нет целей Ч выходим
        if (attackers.Count == 0 || defenders.Count == 0)
            return;

        // ищем приоритетную цель среди живых, у которых есть пассивна€ AbilityData.isPriorityTarget
        ...
        foreach (var atk in attackers)
        {
            ...
            // атаку€сь, отраженный урон тоже применитс€ через TakeDamage в target,
            // но если нужно, можно отдельно:
            atk.TakeDamage(target.cardData.attack);
            atk.OnDamaged();

            // если priority погиб Ч сбрасываем
            if (priority != null && !priority.IsAlive)
                priority = null;
        }
